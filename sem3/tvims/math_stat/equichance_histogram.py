...
for j in range(M):
    ...
    widths.append(hj)
    heights.append(fj)
    centers.append((aj + bj) / 2)

# 5. Построение графика
plt.figure(figsize=(10, 6))
plt.bar(centers, heights, width=widths, edgecolor='black', alpha=0.7, label='Гистограмма f*(x)')

# Оформление
plt.title('Равновероятностная гистограмма', fontsize=14)
plt.xlabel('X', fontsize=12)
plt.ylabel('Плотность вероятности f*(x)', fontsize=12)
plt.xticks(np.round(boundaries, 2), rotation=45) 
